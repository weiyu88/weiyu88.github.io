...
;(function (factory) {
	...
	// Focus
	base.enableFocus = true;
	base.enableLostFocus = false;
	...
	base.init = function () {
		...
		/**
	     * @Description: Callbacks for different events
	     * @Created: Default - Plugin
	     * @Modified:
	     *     Kevin Tan - 08/09/2017 - Added the focus and lost focus events
	     */
		...
		base.$el.on('blur', function (e) { 
			// Check if it's the current element
			if (base.enableLostFocus) {
				// Trigger the lost focus event
				base.$el.trigger(kbevents.inputLostFocus);
				base.enableLostFocus = false;
			}
		});
	};
	...
	base.reveal = function (redraw) {
		...
		base.$el.trigger(kbevents.inputFocus);
		base.enableFocus = false;
		base.enableLostFocus = true;
	...
	base.insertText = function (txt) {
		...
		// Check if it's to reenter the values - linked to emobiqApp.js
		if ($.inputClear) {
			// Set the values
			val = txt;
			t = 1;
		...
		// Vibrate the phone if blank only
		if (vibrate && txt != '' && !isBksp) {
		...
		if ($.isFunction(o.switchInput)) {
			...
			if (!stopped) {
				...
				if (!(kb && kb.options.openOn.length)) {
					isAccepted = base.close(isAccepted, true);
				} else {
	...
	base.close = function (accepted, fullClose) {
		if (base.isOpen && base.$keyboard.length) {
			...
			if (!o.alwaysOpen) {
				...
				base.$el.trigger(kbevents.kbBeforeClose, [base, base.el, (accepted || false), (fullClose ? fullClose : false)]);
			}
	...
	/**
     * @Description: Customized input Events
     * @Created: Default - Plugin
     * @Modified:
     *     Kevin Tan - 08/09/2017 - Added the focus and lost focus events
     */
	...
	$keyboard.keyaction = {
		accept: function (base) {
			base.close(true, true); // same as base.accept();
		...
		tab: function (base) {
			...
			if (tag === 'INPUT') {
				...
				if (o.tabNavigation) {
					return base.switchInput(!base.shiftActive, true, true);
				} else {
	...
	$keyboard.language = {
		en: {
			display: {
				...
				'accept': 'Close:Close (Shift+Enter)',
				// other alternatives \u2311
				'alt': 'Alt:\u2325 AltGr',
